...
<img src="../assets/images/netmaker.png" alt="Netmaker" width="200px"/>

## Introduction

Netmaker is a comprehensive network monitoring and management tool designed to provide administrators with real-time insights into the performance, availability, and security of their network infrastructure. With its intuitive interface and powerful features, Netmaker empowers users to proactively monitor network traffic, detect and troubleshoot issues, and ensure the optimal operation of their network environment. Key features include real-time monitoring, alerting and notifications, traffic analysis, security monitoring, configuration management, and scalability. Overall, Netmaker offers administrators the tools and insights they need to ensure the reliability, performance, and security of their network infrastructure at all times.

## Purpose

Here we are using Netmaker to:
...
Before deploying Netmaker, ensure the target server meets the following prerequisites:

- Public IP address
- Docker and Docker Compose installed
- Keycloak Installed to configure authentication for netmaker
- Optional: Traefik installed to expose Netmaker
...
5. Fill in the form with the following values and click Next:
   ...
   - Client ID: `netmaker-client`
   - Name: `Netmaker Application Client`
...
7. Next Screen
   - Root URL: `https://netmaker.<YOUR-PARENT-DOMAIN>`
   - Valid redirect URIs:
      - `https://netmaker.<YOUR-PARENT-DOMAIN>/*`
      - `http://localhost:53000`
   - Valid post logout redirect URIs: `https://netmaker.<YOUR-PARENT-DOMAIN>/*`
   - Web origins: `+`
8. Click `Save`
...
16. Fill ill in the form with the following values:
   - Name: `Audience for Netmaker Management API`
   - Included Client Audience: `netmaker-client`
   - Add to access token: `on`
   - Add to token introspection: `on`
17. Click `Save`
18. Click `Clients` (left-hand menu)
19. Choose `netmaker-clien`t from the list
20. Switch to `Client scopes` tab
21. Click `Add client scope` button
22. Choose `netmaker-api`
...
26. Fill in the form with the following values and click Next:
   - Client Type: `OpenID Connect`
   - Client ID: `netmaker-backend`
   - Name: `Netmaker Backend Client`
...
1. Update `setup.env` file with the necessary details:
   - `NETBIRD_DOMAIN`: Domain for netmaker. https://netmaker.<YOUR-PARENT-DOMAIN>
   ...
   - `NETBIRD_MGMT_DNS_DOMAIN`: netmaker-net.<YOUR-PARENT-DOMAIN>
...
3. Download the official repository
   ```bash
   REPO="https://github.com/netmakerio/netmaker/"
   # FOR STABLE
...
4. Generate environments parameters:
   ```bash
   cd netmaker/infrastructure_files/
   cp ../../setup.env .
   ./configure.sh
   cd ../..
   cp netmaker/infrastructure_files/artifacts/management.json docker-static/netmaker-mgmt/management.json
   cp netmaker/infrastructure_files/artifacts/turnserver.conf docker-static/netmaker-coturn/turnserver.conf
   cat netmaker/infrastructure_files/artifacts/docker-compose.yml
   ```
...
6. Run the pre-deployment script:
   ...
   ./pre-deployment.sh
   ```
7. Deploy netmaker using Docker Compose:
   ```bash
   docker-compose up -d
   ```

## Post Deployment Steps
1. Open https://netmaker.<YOUR-PARENT-DOMAIN> and login with SSO
